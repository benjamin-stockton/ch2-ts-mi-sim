...
N_sim <- 200 # Number of simulation iterations
...
saveRDS(x1, file = paste0(out_path, "/sim-results-mar-ts-mi-lm-", Sys.Date(), "-sim_setting-", 7, ".rds"))

x2 <- x1 |> 
    dplyr::bind_rows()
f_out <- paste0(out_path, "/sim-results-mar-ts-mi-lm-sim_setting-", 7, ".csv")
